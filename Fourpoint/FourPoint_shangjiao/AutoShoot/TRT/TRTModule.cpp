...
TRTModule::TRTModule(const std::string &onnx_file) {
    ...
    std::cout << "[INFO]: input dims " << input_dims.d[0] << " " << input_dims.d[1] << " " << input_dims.d[2] << " " << input_dims.d[3] << std::endl;
    std::cout << "[INFO]: output dims "<< output_dims.d[0] << " " << output_dims.d[1] << " " << output_dims.d[2] << std::endl;
}
...
std::vector<bbox_t> TRTModule::operator()(const cv::Mat &src) const {
    ...
    for (int i = 0; i < TOPK_NUM; i++) {
        ...
        if (box_buffer[8] < inv_sigmoid(0.5)) break;
        ...
        cv::Rect2f bbox1;
        bbox1.x = reduce_min(box_buffer[0],box_buffer[2], box_buffer[4], box_buffer[6]);
        bbox1.y = reduce_min(box_buffer[1], box_buffer[3], box_buffer[5], box_buffer[7]);
        bbox1.width = reduce_max(box_buffer[0], box_buffer[2], box_buffer[4], box_buffer[6]) - bbox1.x;
        bbox1.height = reduce_max(box_buffer[1], box_buffer[3], box_buffer[5], box_buffer[7]) - bbox1.y;

        cv::rectangle(temp,bbox1,cv::Scalar(0,0,255),1);
        ...
        for (int j = i + 1; j < TOPK_NUM; j++) {
            ...
            if (box2_buffer[8] < inv_sigmoid(0.5)) break;
